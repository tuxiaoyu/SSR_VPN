...
//菜单的显示区域占屏幕宽度的百分比
static CGFloat MenuWidthScale = 0.4f;
//遮罩层最高透明度
...
-(void)showLeftViewControllerAnimated:(BOOL)animated{
    ...
    _coverView.hidden = false;
    [UIView animateWithDuration:[self animationDurationAnimated:animated] animations:^{
        _rootViewController.view.center = CGPointMake(_rootViewController.view.bounds.size.width/2 + 0.8f * self.view.bounds.size.width, _rootViewController.view.center.y);
...
-(void)showRightViewControllerAnimated:(BOOL)animated{
    ...
    [UIView animateWithDuration:[self animationDurationAnimated:animated] animations:^{
        ...
        _coverView.alpha = MaxCoverAlpha;
        [[NSNotificationCenter defaultCenter] postNotificationName:@"RefreshController" object:nil];
...
-(CGFloat)menuWidth{
    ...
    if (CGRectGetMinX(_rootViewController.view.frame) > 0) {
        //左菜单
        return 0.8f * self.view.bounds.size.width;
    }
