...
// Indicate that recent requests need update
#define REQUEST_CACHED @"requestsCached"
...
- (void)updateUserDefaults {
    [[SHVPN sharedUserDefaults] removeObjectForKey:REQUEST_CACHED];
    [[SHVPN sharedUserDefaults] synchronize];
    [[Settings shared] setStartTime:[NSDate date]];
}

- (void)setupWormhole {
    self.wormhole = [[MMWormhole alloc] initWithApplicationGroupIdentifier:[SHVPN sharedGroupIdentifier] optionalDirectory:@"wormhole"];
...
- (void)setupStatusSocket {
    ...
    [self.statusSocket performBlock:^{
        ...
        [[SHVPN sharedUserDefaults] setObject:@(port) forKey:@"tunnelStatusPort"];
        [[SHVPN sharedUserDefaults] synchronize];
    }];
}
...
- (void)startIsland {
    [self syncStartProxy:@"shadowsocks" completion:^(dispatch_group_t g, NSError *__autoreleasing *proxyError) {
        [[ProxyManager sharedManager] startIsland:^(int port, NSError *error) {
            *proxyError = error;
...
- (void)startPacketForwarders {
    ...
    [self startVPNWithOptions:nil completionHandler:^(NSError *error) {
        if (error == nil) {
            ...
            [TunnelInterface startHeaven:[ProxyManager sharedManager].socksProxyPort];
...
- (void)startVPNWithOptions:(NSDictionary *)options completionHandler:(void (^)(NSError *error))completionHandler {
    NSString *generalConfContent = [NSString stringWithContentsOfURL:[SHVPN sharedGeneralConfUrl] encoding:NSUTF8StringEncoding error:nil];
...
- (void)openLog {
    NSString *logFilePath = [SHVPN sharedLogUrl].path;
...
- (void)stop {
    NSLog(@"stoping Super tunnel...");

    [[SHVPN sharedUserDefaults] setObject:@(0) forKey:@"tunnelStatusPort"];
    [[SHVPN sharedUserDefaults] synchronize];
    [[ProxyManager sharedManager] stopHttpProxy];
    [[ProxyManager sharedManager] stopSocksProxy];
    [TunnelInterface stopHeaven];

    [[ProxyManager sharedManager] stopIsland];
}
...
- (void)sleepWithCompletionHandler:(void (^)(void))completionHandler {
    NSLog(@"sleeping Super tunnel...");
    completionHandler();
}

- (void)wake {
    NSLog(@"waking Super tunnel...");
}
